...
function main_func()
	...
	#init_upper_val, init_upper = estimate_upper(config, params, demand_error, coefs, elite_pop) #Upper bound for horizon
	...
	@printf "Instr_cr_change: %.7f Instr_mu_change: %.7f\n" instr_cr_change instr_mu_change 
	...
	upper_val::Float64 = init_upper_val
	lower_val::Float64 = -400
	
	#False position method 

	mid = keep_five((lower*upper_val - upper*lower_val) / (upper_val - lower_val))
...
## TODO
	...
	while abs(upper - lower) > epsilon
		...
		for test in 1:no_tests
			...
			if abs(best_error) > abs(curr_error)
		...
		if trial_error * lower_val > 0
			lower_val = trial_error
		else
			upper_val = trial_error
		end
